...
VerifyShaOfStage3()
{
	# Format is different for funtoo.
	test_sum=$(awk '{print $2; exit}' $2)
	calculated_sum=$(sha256sum $1 | awk '{print $1}' -)
	echo "test = $test_sum"
	echo "calc = $calculated_sum"
...
arch=$1
# Note that the funtoo.org site seems to not work with the wget from
# busybox.
stage3path="http://ftp.osuosl.org/pub/funtoo/funtoo-current/x86-64bit/intel64-haswell/stage3-latest.tar.xz"
...
wget -q -c "${stage3path}" "${stage3path}.hash.txt"
if VerifyShaOfStage3 $stage3 "${stage3}.hash.txt"; then
	echo "DIGEST sum is okey";
else
	echo "DIGEST sum is NOT okey";
	return 1;
fi
xz -d < ${stage3} | tar --exclude "./etc/hosts" --exclude "./sys/*" -xf -
